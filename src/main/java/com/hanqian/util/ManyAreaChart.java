...
public class ManyAreaChart {
	...
	public ManyAreaChart(List<ChartData> list,String title){
		this.list = list;
		this.title = title;
	}
	...
	private Paint[] createPaint()
	{
		...
		if(null !=list){
			...
			for (int i =0; i< list.size(); i++) {				
					...
					Color color=null;
					if(cmap.get(list.get(i).getId())==null || list.get(i).getId().equals("0")){
							color = new Color((int)(Math.random()*255),(int)(Math.random()*255),(int)(Math.random()*255));
					}else{
							color = cmap.get(list.get(i).getId());
					}
					apaint[i] = color;				
			}
