 ...
 *
 */
public class BusinessAtoShowDate {
	/**
	 * log4g日志
	 */
	private static final Log logg = LogFactory.getLog(BusinessAtoShowDate.class);
	...
	public BusinessAtoShowDate(Short powerId) {
		super();
	 ...
	 * @return
	 */
	public List<ChartData> getChartData(List rlist,String valKey,String valUnit,Short power){
		...
		if (null != rlist) {
			...
			for(int i=0;i<rlist.size();i++){
				...
				rData.setMapTitle(name+stattime+powerName+"能源"+valUnit+"="+val);
				data.add(rData);
			}
		}
	...
	public TableData getTableDate(List<ChartData> clist,String unit,
		...
		try {
			...
			logg.error("BusinessAtoShowDate-->getTableDate", e);
					...
					for (String key:cllist) {
						...
						if (bval != null) {
							if (val>bval+(pCha/100)) {
								vls.add("<font  color='#FF0000' title='高于标准("+bval+")\n偏差范围' style='cursor:pointer'>"+val+"</font>");
							}else if(val+(pCha/100)<bval){
								vls.add("<font  color='#FF0000' title='低于标准("+bval+")\n偏差范围' style='cursor:pointer'>"+val+"</font>");
							}else{
								vls.add(val);
							}
						}else{
