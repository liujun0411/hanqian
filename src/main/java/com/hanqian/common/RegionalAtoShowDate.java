 ...
 *
 */
public class RegionalAtoShowDate {
	/**
	 * log4g日志
	 */
	private static final Log logg = LogFactory.getLog(RegionalAtoShowDate.class);
	...
	public RegionalAtoShowDate(Short powerId) {
		super();
	...
	public TableData getTableDate(List<ChartData> clist,String unit,
		...
		try {
			...
			logg.error("RegionalAtoShowDate-->getTableDate", e);
					...
					for (String key:cllist) {
						...
						if (bval != null) {
							if (val>bval+(pCha/100)) {
								vls.add("<b><font  color='#FF0000' title='高于标准("+bval+")\n偏差范围' style='cursor:pointer'>"+val+"</font></b>");
							}else if(val+(pCha/100)<bval){
								vls.add("<b><font  color='#7030A0' title='低于标准("+bval+")\n偏差范围' style='cursor:pointer'>"+val+"</font></b>");
							}else{
								vls.add(val);
							}
						}else{
