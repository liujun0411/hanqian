		...
		var _toolbarStr = "\
			<div id='EDiaryEditor'style='width: 621px;'>\
				...
				<div style='width: 621px; height: 59px; background-image: url(images/editor/editor_foot.gif)'>\
					<div style='float: left; padding-top: 18px; margin-left: 18px; margin-top: 16px;'><input type='checkbox' id='ModeCheck'><label for='ModeCheck'>显示源代码</label>&nbsp;&nbsp;&nbsp;当前字数:<font color='#FF0000'><span id='nowCharCount'>0</span></font></div>\
				</div>\
			</div>\
		";
	...
	initControl: function (bWordMode, sContent) {
        ...
        function eventObserver(){
            var es =_this.iframe.contentWindow.document.body.keyupEvents;
				/*
				 * 字体预设 if(es.fontsize){ reaplceFontName("fontsize",es.fontsize) }
				 */
                if(es.fontname){
		...
		if(CMD == "" || CMD == null)return;
			...
			for(var i = 0; i < oChild.length; i++){
				if(oChild[i].tagName){
					...
					// 是否已经排过版,使用 2em 会使段落排版混乱［h1,h2混］,可设计默认 28 像素。通过计算子节点
					// fontSize 方式同样有此问题
