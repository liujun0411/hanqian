...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			 ...
			 *更新备注/消除告警
			 */
			var updataRemarks=function(action,sequence,startTime,deleteflag,updateDate){
				var mydata = new Array();
				mydata[0] = action;
				mydata[1] = sequence;
				mydata[2] = startTime;                                                                                 
				var returnValue = window.open("<%=path%>/manager/monitoring/control/monitoringPlatform/alarmRemarks.jsp?action="+action+"&sequence="+sequence+"&startTime="+startTime+"&updateDate="+updateDate,mydata,"height=600, width=400, toolbar =no, menubar=no, left=450,top=20,scrollbars=no, resizable=no, location=no, status=no");
				//"center:yes;dialogHeight:580px;dialogWidth:450px;status:no;"
				
			}
			function vodereturnValue(){
			  	var urlstr = "alarm_findCurrentAlarmInfoLess.action?eqcurrentPageLess=1";
			...
			var formCheck = function(currentPage){
				if(currentPage != ""){
					$("#eqcurrentPageLess").val(currentPage);
				}
	...
	<body style=" background-color: white;">
          <form id="myform" name="myform" action="alarm_findCurrentAlarmInfoLess.action">
			<span class="admin_clor_f00">${outMsg}</span>
			<input type="hidden" id="level" name="level" value="${levelLess}"/>
			<input type="hidden" id="equipId" name="equipId" value="${equipId}"/>
			<input type="hidden" id="groupId" name="groupId" value="${groupId}"/>
			<input type="hidden" id="eqcurrentPageLess" name="eqcurrentPageLess" value="${eqcurrentPageLess}"/>
			<input type="hidden" id="flag" name="flag" value="${flag }"/>
		  </form>
			<c:if test="${!empty listAlertsCurrentLess}">
...
<%--						</th>--%>
				...
				<tbody id="">					
						<c:forEach items="${listAlertsCurrentLess}" var="obj" varStatus="indx">
...
<%--								</c:if>						--%>
								...
								<td align="center" class="biao_lianj_1">
									<c:if test="${!empty menuIdMap && !empty menuIdMap['4001001001']}">
										<a href="javascript:void(0);" onclick="updataRemarks('alarm_updateRemarks','${obj.seq }','<f:formatDate value='${obj.starttime}' pattern='yyyy/MM/dd HH:mm:ss' />','','<f:formatDate value='${obj.updateDate}' pattern='yyyy/MM/dd HH:mm:ss' />');">查看</a>
								    </c:if>
<%--								</td>--%>
<%--								<td align="center" class="biao_lianj_1">--%>
									<c:if test="${!empty menuIdMap && !empty menuIdMap['4001001002']}">
									 <a href="javascript:void(0);" onclick="updataRemarks('alarm_removeAlarm','${obj.seq }','<f:formatDate value='${obj.starttime}' pattern='yyyy/MM/dd HH:mm:ss' />','${obj.deleteflag}','<f:formatDate value='${obj.updateDate}' pattern='yyyy/MM/dd HH:mm:ss' />');">消除</a>
									</c:if>
								</td>								
							</tr>
