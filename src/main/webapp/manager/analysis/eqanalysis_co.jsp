...
<body onload="initOnload('${voeqa.power }');" style="overflow:scroll;overflow-x:hidden;overflow-y:hidden;">
		...
		<!-- form 表单 -->
		<form method="post" action="eqcoanalyssis_showEqCoAnalysisUI.action" name="myform"	id="myform">
	       ...
	       <tr class="shebeigl_inp_zt">
				...
				设备类型:
	         	...
	         	&nbsp;
	       		...
	       		<td class="biao_lianj_1" ><a href="javascript:void(0);" onclick="$('#xtype').show();">生产厂商</a></td>
		...
		myObject.prototype.getCo=function(id){
			...
			for(var i=0;i<this.aNodes.length;i++){
				if(this.aNodes[i].id==id){
					...
					'value="'+this.aNodes[i].name+'" id="ch'+i+'"/><label id="lb'+i+'" for="ch'+i+'" onmouseover="onover(this);" onClick="oncli(this);" onmouseout="onout(this);">'+this.aNodes[i].name+'</label><br/>';
					count +=1;
				}
			}
	...
	<c:forEach items="${selobj.prList}" var="obj" >
		<script>
			co.add("${obj.eqtype}","${obj.name}","");
		</script>
	</c:forEach>
	...
	<script>
		...
		//选择类型 
		getMyChild=function(eqtype,stypid){
			//$("#eqType").val(eqtype);
			if(""!=eqtype){
				$("#eqTypeChilds").html(obj.getPidSel(eqtype,stypid));
				if (null == $("#subsel").val()) {
					getCo(eqtype);
				}else{
					getCo($("#subsel").val());
