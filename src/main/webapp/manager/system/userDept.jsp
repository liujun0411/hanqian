...
function insertSubmit() {
	...
	var deptId = document.getElementById('deptId').value;
	   ...
	   $('#roleId1 option').each(function(i){
	    ...
	    //给userId赋值
	    //$("#userSeq").val(userSeq);
	var url = 'dept!userDepteUpdate.action?dbDept.seq=' + deptId;
	for(i=0;i<userSeq.length;i++){
		url+='&userSeq='+userSeq[i];
	}
			...
			<form action="" method="post" id="myform">
				<table style="width:70%;font-weight: lighter;">
					<tr>
						<th align="left" colspan="6" class="tetle_font">
							组织基本信息
							<input type="hidden" name="deptId" id="deptId" value="${dbDept.seq}" />
						</th>
					</tr>
					<tr>
						<th align="right" class="font_size_color">组织名:</th>
						<td class="font_size_color">${dbDept.name}</td>
						<th align="right" class="font_size_color">组织负责人:</th>
						<td class="font_size_color">${dbDept.principal}</td>
					</tr>
				</table>
				<br/>
				<br/>
				...
				<div class="admin_nr_rightg">
					<table style="width: 70%;font-weight: lighter;">
						...
						<tr>
							<th align="left" class="font_size_color">可选用户</th>
							<th></th>
							<th align="left" class="font_size_color">已选用户</th>
						</tr>
						<tr>
							...
							<select multiple="multiple" id="userId1"  ondblclick="userRole_select_dblclick_right()"  class="select_css">
								         <c:if test="${!empty notDeptUserLst}">
												<c:forEach items="${notDeptUserLst}" var="u">
							...
							<td style="height: 50px; width: 40%;" align="left" class="lm_dlg"> 
								<select multiple="multiple" id="roleId1" ondblclick="userRole_select_dblclick_left()" class="select_css">
							           <c:if test="${! empty deptUserLst}">
												<c:forEach items="${deptUserLst}" var="u">
												  <option value="${u.seq}">${u.username}</option>
												</c:forEach>
							           </c:if>
								</select>
						...
						<tr>
							<td colspan="3" align="right">
								<input type=reset styleClass="common_button"
									onclick="javascript:history.go(-1);" value="取 消" class="btn blue">
								<input type="button" onclick="insertSubmit();"
									styleClass="common_button" value="保 存" class="btn blue"/>
							</td>
						</tr>
					</table>
					<br>
