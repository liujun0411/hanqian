...
$.TextboxListBit = function(type, value, textboxlist, _options){
	...
	var init = function(){
		...
		if (type == 'box'){
			...
			element = $('<input type="text" class="'+ typeprefix +'-input" placeholder="请填写提示信息." autocomplete="off" />').val(self.value ? self.value[1] : '').appendTo(bit);
	...
	var inject = function(el, where){
		...
		textboxlist.onAdd(self);if (options.growing) element.data('growing').resize();	
