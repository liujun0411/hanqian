...
iChart.Gauge2D = iChart.extend(iChart.Chart, {
	...
	doAnimation : function(t, d,_) {
		...
		var v1 = _.animationArithmetic(t,_.needleOne.start, _.needleOne.offset, d); //jiang
		var v2 = _.animationArithmetic(t,_.needleTwo.start, _.needleTwo.offset, d); //jiang
		_.panel.draw();
		...
		_.needleOne.push('value',v1); //jiang
		_.needleTwo.push('value',v2); //jiang
		_.needle.draw();
		_.needleOne.draw(); //jiang
		_.needleTwo.draw(); //jiang
	},
    /**
	 * get value by jiang
	 */
	to:function(value,value2,value3){
		var _ = this._();
		if(value==_.needle.value)return;
		if(value2==_.needleOne.value)return; //jiang
		if(_.processAnimation){
			_.needle.start = _.needle.get('value');
			_.needleOne.start = _.needleOne.get('value'); //jiang
			_.needleTwo.start = _.needleTwo.get('value'); //jiang
		}else{
			_.needle.start = _.needle.value;
			_.needleOne.start = _.needleOne.value; //jiang
			_.needleTwo.start = _.needleTwo.value; //jiang
		}
		_.needle.value = parseFloat(value);
		_.needleOne.value = parseFloat(value2); //jiang
		_.needleTwo.value = parseFloat(value3); //jiang
		_.needle.offset = _.needle.value-_.needle.start;
		_.needleOne.offset = _.needleOne.value-_.needleOne.start; //jiang
		_.needleTwo.offset = _.needleTwo.value-_.needleTwo.start; //jiang
		_.runAnimation(_);
	},
	...
	doConfig : function() {
		 ...
		 * build needleOne
		 */
		_.needleOne = new iChart.Custom({
			z_index:_.get('z_index')-5,
			radius:iChart.parsePercent(_.get('needle_radius'),_.panel.tr - _.get('tickmarks_width')*0.5),
			originx:_.x,
			originy:_.y,
			panel:_.panel,
			value:value,
			size:_.get('needle_sizeOne'),
			background_color:_.get('needle_colorOne'),
			cap:{
				size:_.get('center_cap_sizeOne'),
				color:_.get('center_cap_color')
			},
			getRadian:function(v){
				var l = _.panel.lower,u = _.panel.upper;
		 ...
		 * build needleTwo
		 */
		_.needleTwo = new iChart.Custom({
			z_index:_.get('z_index')-4,
			radius:iChart.parsePercent(_.get('needle_radius'),_.panel.tr - _.get('tickmarks_width')*0.5),
			originx:_.x,
			originy:_.y,
			panel:_.panel,
			value:value,
			size:_.get('needle_sizeTwo'),
			background_color:_.get('needle_colorTwo'),
			cap:{
				size:_.get('center_cap_sizeTwo'),
				color:_.get('center_cap_color')
			},
			getRadian:function(v){
				var l = _.panel.lower,u = _.panel.upper;
		...
		_.components.push(_.needleOne); //new needleOne by jiang
		_.components.push(_.needleTwo); //new needleOne by jiang
	}
