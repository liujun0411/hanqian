...
public class BiLiChart {
	...
	public BiLiChart(List<ChartData> list,String title){
		this.list = list;
		this.title =title;
	}
...
//		categoryplot.setBackgroundAlpha(1f);  //数据区的背景透明度 1不透明 ,0 透明		 
	...
	private CategoryToolTipGenerator[][] createToolTip(){
		...
		if (null != list) {
			tools=new CategoryToolTipGenerator[2][11];
			for (int i = 0,j=1; j < list.size(); i++,j+=2) {	
				tools[0][i]=new StandardCategoryToolTipGenerator(list.get(j-1).getMapTitle(),NumberFormat.getPercentInstance());
				tools[1][i]=new StandardCategoryToolTipGenerator(list.get(j).getMapTitle(),NumberFormat.getPercentInstance());				
	...
	private Paint[][] createPaint()
	{
		Paint[][] apaint = null;
		if(null !=list){
			apaint = new Paint[2][11];
			
			for (int i = 0,j=1; j < list.size(); i++,j+=2) {				
					HashMap<String,Color> cmap = ColorLibray.getColorMap();
					Color color=null;
					if(cmap.get(list.get(j).getId())==null){
							color = new Color((int)(Math.random()*255),(int)(Math.random()*255),(int)(Math.random()*255));
					}else{
							color = cmap.get(list.get(j).getId());
					}
					//标准
					apaint[0][i] = color;
					//实际
					apaint[1][i] = color;//new Color((int)(Math.random()*255),(int)(Math.random()*255),(int)(Math.random()*255));
			}
