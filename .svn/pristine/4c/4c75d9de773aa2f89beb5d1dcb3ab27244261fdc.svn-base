...
<body>
	<div id="admin_nr_rightg">
		...
		<div id="ListInfo">
			<div class="itemList2" style="height:800px;overflow-y:scroll">
				<table width="100%" border="0" cellspacing="1" class="listTable">
					...
					<tbody id="stu-datas-tb">
						<c:forEach items="${heatPumpSystemList}" var="item" varStatus="idx">
							<tr class="tr${idx.index%2+1}" id="trhref">
								<td align="center">${idx.index+1}</td>
								<td align="center">
									<a href="javascript:void(0);" style="color: black">${item.DESCR == null ? "--" : item.DESCR}</a>
								</td>
								<td align="center">${item.RECORD == null ? "--" : item.RECORD}</td>
								<td align="center">
									<c:choose>
										<c:when test="${item.RECORDTIME == null}">
											--
										</c:when>
										<c:otherwise>
											<f:formatDate value="${item.RECORDTIME}" pattern="yyyy-MM-dd HH:mm:ss" />
										</c:otherwise>
									</c:choose>
								</td>
							</tr>
