            ...
            <table width="100%" border="0" cellspacing="1"
                ...
                <tbody id="stu-datas-tb">
                    <c:forEach items="${listEquipList}" var="equipment" varStatus="idx">
                        ...
                        <td align="center">
						    <c:if test="${equipment.da==0}">--</c:if>
							<c:if test="${equipment.da==1}">开</c:if>
							<c:if test="${equipment.da==2}">关</c:if>
							<c:if test="${equipment.da==3}">不确定</c:if>
							<c:if test="${equipment.da >=4 || equipment.da <0}">--</c:if>
                        </td>
                                                    ...
                                                    <span>${equipService.server_building_name }</span>
                                                     <c:if test="${!empty equipment.storey_num_up}">
                                                        <c:if test="${equipService.server_storey<= equipment.storey_num_up&&equipService.server_storey>0}">
                                                             ${equipService.server_storey }
                                                        </c:if>
                                                         <c:if test="${equipService.server_storey>equipment.storey_num_up}">
                                                              <font size="2px;">屋</font>顶
                                                         </c:if>
                                                     </c:if>
                                                    <c:if test="${equipService.server_storey<0}">
                                                         B${equipService.server_storey<0?-equipService.server_storey:equipService.server_storey}
                                                    </c:if>
                                                    <c:if test="${!empty equipService.server_storey&&equipService.server_storey<=equipment.storey_num_up}">
                                                        层
                                                    </c:if>
                                                <span>${equipService.server_areas}</span>
                                                        <span>${equipService.server_energy}</span>
                                                <span>  <c:if test="${!empty equipService.server_comments}">(</c:if>
                                                    ${equipService.server_comments}
                                                    <c:if test="${!empty equipService.server_comments}">)</c:if>
                                                    </span>
                                                <span><c:if test="${!empty equipService.server_square}">
