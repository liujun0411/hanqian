...
<%--                                </c:forEach>--%>
                        ...
                        <c:if test="${eqTypeId==10001 || eqTypeId==10002||eqTypeId==12001}">
                            <td width="12%" align="right">
                                服务楼宇：
                            </td>
                            <td>
                                <select id="serviceBuildId" name="serviceBuildId" style="width: 150px;" onchange="getStoreyByBuildId('serviceBuildId','serviceStorey','${serviceStorey }','');">
                                    <option value="">
                                        --请选择楼宇--
                                    </option>
<%--                                    <c:forEach items="${dbBuildingService}" var="build">--%>
<%--                                        <option value="${build.buildingId}" ${build.buildingId==serviceBuildId?'selected' : ''}>--%>
<%--                                            ${build.buildingName}--%>
<%--                                        </option>--%>
<%--                                    </c:forEach>--%>
                                </select>
                            ...
                            <td style="width: 12%">
                                <input type="text" name="serviceStorey" id="serviceStorey" maxlength="2"
                                 disabled="disabled"   value="${serviceStorey}" onkeyup="value=value.replace(/[^\-?\d.]/g,'')" />
                            </td>
                        </c:if>
                        <c:if test="${eqTypeId!=10001&&eqTypeId!=10002&&eqTypeId!=12001}">
                            <td>
                    ...
                    <tr>
                        <c:if test="${eqTypeId==10001 || eqTypeId==10002||eqTypeId==12001}">
                            ...
                            <td width="12%">
                                <input type="text" name="serviceAreas" id="areas" value="${serviceAreas }" />
                            </td>
                        </c:if>
                            <td align="right" height="10px" width="12%"
                    ...
                    <tr>
                        <c:choose>
                            <c:when test="${eqTypeId == 10001}">
                                ...
                                <c:if test='${sessionScope.currentHospCode == \'ETYY\' }'>
                                </c:if>
                                <c:if test='${sessionScope.currentHospCode != \'ETYY\' }'>
                                        onclick="goPowerControl(${equipment.equipid},${eqTypeId})"
                                </c:if>       
                                    >
                            </c:when>
                        ...
                        <td align="center" nowrap="nowrap">
                            <c:if test="${!empty equipment.storey_num_up}">
	                            <c:if test="${equipment.storey<= equipment.storey_num_up&&equipment.storey>0}">${equipment.storey }</c:if>
                                <c:if test="${equipment.storey>equipment.storey_num_up}">屋顶</c:if>
                            </c:if>
                            <c:if test="${equipment.storey<0}">B${equipment.storey<0?-equipment.storey:equipment.storey}</c:if>
                        </td>
                        <c:if test="${eqTypeId==10001 || eqTypeId==10002||eqTypeId==12001}">
                            <td align="center">
                             <c:forEach items="${equipment.server}" var="equipService">
                                <c:if test="${!empty equipService}">
                                   <span>${equipService.server_building_name }</span>
                                   <c:if test="${!empty equipment.storey_num_up}">
                                   <c:if test="${equipService.server_storey<= equipment.storey_num_up&&equipService.server_storey>0}">
                                   ${equipService.server_storey }</c:if>
                                   <c:if test="${equipService.server_storey>equipment.storey_num_up}">
                                   <font size="2px;">屋顶</font>
                                   </c:if></c:if><c:if test="${equipService.server_storey<0}">
                                   B${equipService.server_storey<0?-equipService.server_storey:equipService.server_storey}</c:if>
                                   <c:if test="${!empty equipService.server_storey&&equipService.server_storey<=equipment.storey_num_up}">
                                                                                                                           层</c:if><span>${equipService.server_areas}</span><span>${equipService.server_energy}</span>
                                   <span><c:if test="${!empty equipService.server_comments}">(</c:if>
                                   ${equipService.server_comments}<c:if test="${!empty equipService.server_comments}">)</c:if>
                                   </span><span><c:if test="${!empty equipService.server_square}">
                                   ${equipService.server_square}㎡
                                   </c:if></span><br/></c:if>
                            </c:forEach>
                            </td>
                            <td align="right">
                            <c:choose>
                            <c:when test="${equipment.data_value == '0'}">
                                    			 <f:formatNumber type="number" maxFractionDigits="2" value="${equipment.data_value }"/> ${equipment.unit }
                              </c:when>
                                <c:when test="${!empty equipment.data_value}">
                                    <f:formatNumber type="number" maxFractionDigits="2" value="${equipment.data_value }" pattern="#,#00.00"/> 
                                    <c:choose>
                                        <c:when
                                            test="${!empty equipment.data_value && (eqTypeId == 10002||eqTypeId==12001)}">
                                                    m³
                                        </c:when>
                                        <c:otherwise>
                                            <c:if test="${!empty equipment.data_value }">${equipment.unit }</c:if>
                                        </c:otherwise>
                                    </c:choose>
                                </c:when>
                                <c:otherwise>
