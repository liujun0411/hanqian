...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
		toAdd = function() {
            location.href = "maintenance_addMaintenanceUI.action";
        }
...
$(function() {
	...
	$(".deleM").click(function(event){
		...
		if(window.confirm('是否删除维护信息?')){
			window.location='maintenance_deleteMaintenance.action?mid='+langstr;
		}
...
trClick = function(eqId) {
	$('#myform').attr( {
		action : "equipment_showDetail.action?equipId=" + eqId
	});
	document.myform.submit();
}
     //点击所在楼宇“--请选择--”所在楼层是灰掉
	...
	<body>
		<div id="admin_nr_jbxx">
			...
			<form action="maintenance_findMaintenance.action" method="post"
				...
				<table width="100%" border="0"
					...
					<tr style="height: 30px" valign="middle">
						...
						<td width="150">
							<input id="storey" name="storey" value="${voMaintenance.storey }" maxlength="2" disabled="disabled"
								onkeyup="value=value.replace(/[^\-?\d.]/g,'')" type="text" style="width: 120px;" />
						</td>
						...
						<td width="250" colspan="2">
							<input id="d4311" size="12"
								...
								style="width: 85px;"
								onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'d4312\')||\'2020-10-01\'}',onpicked:function(){d4312.focus();}})" />
							至
					...
					<tr style="height: 30px" valign="middle">
						...
						<td width="80">
						      <select id="voMaintenance.maintenancetype" name="voMaintenance.maintenancetype" class="myfont" >
								<option value=""> --请选择类型--</option>
									<option value="1" ${1== voMaintenance.maintenancetype?'selected':''}>维修</option>
									<option value="2" ${2== voMaintenance.maintenancetype?'selected':''}>巡检</option>
									<option value="3" ${3== voMaintenance.maintenancetype?'selected':''}>保养</option>

						</td>
						<td width="90"  colspan="7" align="right">
				...
				<tbody id="stu-datas-tb">
					<c:if test="${!empty mainList}">
						<c:forEach items="${mainList}" var="maint" varStatus="i">
							<tr class=" biaog_kan2 biaog_zt2" style="cursor: pointer;"
								...
								<c:if
									...
									<td align="center" class="biao_lianj_1">
										<c:if test="${!empty menuMap && !empty menuMap['2004001002']}">
											<a href="maintenance_showEditUI.action?mid=${maint.seq }">修改</a>
										</c:if>
